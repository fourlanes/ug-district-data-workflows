...
@flow(name="primary-enrollment-processing", log_prints=True)
def process_primary_enrollment_data(
    input_file: str,
    ...
    """
    Main flow for processing primary school enrollment data and extracting education facilities.

    This flow is specifically designed for primary enrollment data files that contain school
    information and need to be processed to extract unique primary education facilities,
    plus output a cleaned version of the original enrollment data with location codes and facility IDs.

    Args:
        input_file: Path to input CSV file (primary enrollment data)
    ...
    logger.info(f"Starting primary enrollment data processing for: {input_file}")
    ...
    logger.info("Step 1: Loading primary enrollment data...")
    df_original = pd.read_csv(input_file, encoding="utf-8-sig")
    logger.info(f"Loaded {len(df_original)} primary enrollment records from {input_file}")
    ...
    logger.info("Primary enrollment data cleaning completed")
    ...
    if 'parish' in df_original.columns:
    ...
    unique_schools = df_original.groupby(grouping_columns).first().reset_index()
    ...
    education_facilities['parish'] = unique_schools.get('parish', '')  # May be available in primary data
    ...
    logger.info("Step 7a: Saving cleaned primary enrollment data...")
    ...
    logger.info(f"Saved cleaned primary enrollment data with {len(df_enrollment_final)} records to: {enrollment_output_path}")
    ...
    processing_report_path = (
        reports_dir / f"{district_name}_primary_enrollment_{timestamp}_processing_report.json"
    )

    processing_report = {
        "input_file": input_file,
        "processing_type": "primary_enrollment_to_education_facilities",
    ...
    logger.info(
        f"Primary enrollment processing completed successfully. "
        f"Extracted {len(df_final)} primary education facilities from {len(df_original)} enrollment records. "
        f"Saved cleaned enrollment data with {len(df_enrollment_final)} records."
    )
    return results


@flow(name="batch-primary-enrollment-processing")
def batch_process_primary_enrollment_data(input_directory: str = "data/raw/trends") -> dict:
    """
    Process multiple primary enrollment CSV files in batch.

    Args:
        input_directory: Directory containing primary enrollment CSV files to process
    ...
    # Find primary enrollment CSV files (pattern matching)
    csv_files = [
        f for f in input_path.rglob("*.csv")
        if "primary" in f.name.lower() and ("enrollment" in f.name.lower() or "enrolment" in f.name.lower())
    ]
    logger.info(f"Found {len(csv_files)} primary enrollment CSV files to process")

    if not csv_files:
        logger.warning("No primary enrollment CSV files found in input directory")
        return {"status": "warning", "message": "No primary enrollment CSV files found"}
    ...
    for csv_file in csv_files:
        ...
        try:
            result = process_primary_enrollment_data(str(csv_file))
    ...
    logger.info(f"Batch primary enrollment processing completed: {successful} successful, {failed} failed")
    return batch_summary


if __name__ == "__main__":
    # Example usage for single file
    result = process_primary_enrollment_data(
