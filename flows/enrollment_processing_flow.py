...
@flow(name="enrollment-processing", log_prints=True)
def process_enrollment_data(
    ...
    """
    Main flow for processing enrollment data and extracting education facilities.

    This flow is designed for enrollment data files (primary or secondary) that contain school
    information and need to be processed to extract unique education facilities, plus output a
    cleaned version of the original enrollment data with location codes and facility IDs.
    ...
    logger.info(f"Starting enrollment data processing for: {input_file}")
    ...
    # Auto-detect institution type from file name or data structure
    input_filename = Path(input_file).name.lower()
    if "primary" in input_filename:
        institution_type = "Primary"
        logger.info("Detected Primary school enrollment data")
    elif "secondary" in input_filename:
        institution_type = "Secondary"
        logger.info("Detected Secondary school enrollment data")
    else:
    ...
    processing_report_path = (
        reports_dir / f"{district_name}_enrollment_{timestamp}_processing_report.json"
    )

    processing_report = {
        "input_file": input_file,
        "processing_type": "enrollment_to_education_facilities",
    ...
    logger.info(
        f"Enrollment processing completed successfully. "
        f"Extracted {len(df_final)} {institution_type.lower()} education facilities from {len(df_original)} enrollment records. "
        f"Saved cleaned enrollment data with {len(df_enrollment_final)} records."
    )
    return results


@flow(name="batch-enrollment-processing")
def batch_process_enrollment_data(input_directory: str = "data/raw/trends") -> dict:
    """
    Process multiple enrollment CSV files in batch.

    Args:
        input_directory: Directory containing enrollment CSV files to process
    ...
    if not csv_files:
        logger.warning("No enrollment CSV files found in input directory")
        return {"status": "warning", "message": "No enrollment CSV files found"}
    ...
    for csv_file in csv_files:
        ...
        try:
            result = process_enrollment_data(str(csv_file))
